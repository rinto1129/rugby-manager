...
- `p` 選手（`p.wg`=ウエイト時間帯。選手の申告{f5:[5限の曜日],far,pref,upd}＋スタッフだけが書く曜日の指定{ov:{mon,tue,thu:'am'|'pm'|''},ovUpd}。読みは`wgNorm`（実効値days=ov優先→5限は'am'。一時形式{v:2,days}も吸収・移行処理なし。選手の`saveMyWg`はovをサーバー最新から引き継ぐ）/ `i` 怪我 / `r` リハビリ / `ph` フィジカル測定（正式な1RM等。**項目制の測定会の記録は選手×測定会で1件**＝`at:{項目:測定日}`・`by:{項目:'player'|'staff'}`・`ex:{項目:免除理由}`・`submitted/submittedAt/submittedBy`を持つ。この記録を書けるのは`phSessApply/phSessSubmit`と特設ページ化の統合だけ）/ `a` 欠席 / `f` コンディション（フィールドは rpe/sleep/duration/note/weight/durMin/mood/stress/soreness/sorenessParts/editedAt。**体重はfが正**・bcとは同日dedupでbc優先）
...
- `msess` 測定会（{id,name,startDate,endDate,mtype,note,closed,…}＋**`items:[項目キー]`が非空なら「項目制」＝特設ページ・選手ごとの提出**。無ければ従来の測定会。`items`は記録の無い会の編集か「特設ページ化」（記録の統合と不可分）でだけ付く）
...
- 保存層: `svSafe(k,rec,cb)` 新規追記 / `svSafeUpdate(k,fn,cb)` 更新・削除 / `svSafeSeq`（**player/staff/trainer identical**・追記専用チェーン。P1c-1でstaffにも移植済み）/ `chartUpdate(iid,fn,cb)`（staff/trainer・カルテの操作単位更新）。**素のsv(k)は使わない**（staff:1235付近の初回シード1箇所のみ不可侵で残存＝リポジトリ全体で唯一）。
...
- ウエイトグループ分けv2（2026-09・player/staff identical）: `liftKeyOf(exName,estBase)`（種目→'squat'|'bench'|'deadlift'|null。estBase未設定なら種目名推定＝「デット」表記・変種除外。**トレーニング実行/種目追加/推定1RM記録`bestE1rmPerBase`もこれで補完**）／`posUnit(pos)`（'FW'|'BK'・coachも）／`wgNorm`・`wgDayShift`・`wgF5Txt`・`wgOvTxt`（申告＋曜日指定の読み・短い表示）／`tgLiftWeights(pid,logs)`（直近`TG_LIFT_DAYS`=60日のBP/SQ/DL 1セット重量。staffはアーカイブ込み`tlogAll()`）／`WG_DAYS`・`tgLiftTxt`・`tgDayLabel`／曜日表示=`tgDayShifts(shifts,day)`（その曜日の実際の班構成 here/away/unplaced）・`tgDefaultDay`・`tgIsDay`・`tgDayTabsHtml(cur,fn)`（月/火/木タブ。staff=`tgSetDay`/player=`agSetDay`で再描画）。`WG_CHOICES`・`wgSegHTML/wgSegPick/wgSegRead`はstaff専用（曜日の指定 自動/午前/午後）。班分けアルゴリズム（ホーム組=ピン＞指定3日＞5限＞希望＞遠方＞自動・実効値が逆の曜日はゲスト）・手動編集・保存履歴はstaff専用（プラン=dev/audit/PLAN_tgroup_v2.md）。
- 測定会の特設ページ（2026-09-14・プラン=dev/audit/PLAN_msess_page.md）: 読み系（player/staff/coach identical）`PH_ITEMS`(var・7項目カタログ)/`phItem`/`msessItems(s)`（items非空=項目制）/`phHasVal`/`phDateOf`。player/staff identical: `phSessRecs/phSessRec/phSessProgress(s,pid)`（{items:[{k,val,at,ex,by,done}],doneN,total,complete,submitted,unmerged}）/`msessOpenItemsAll()`・`msessOpenItems()`・`msessOpenFor(keys)`（開いている項目制の会=未締切＋開始〜終了+14日。複数が同時に開いていれば測る項目で振り分け）/`phSessFind`/`phSessComplete(r,ks)`/`phSessApply(latest,o)`（**項目制の記録を書く唯一の純粋関数**・差分適用・o.anyでカタログ内なら会の項目外も保存・書くものが無ければ空の記録を作らない・値を消して未完了になれば提出も外す・editedAtは付けない）/`phSessSubmit`。**`phInMSess`は項目制の会では msessId 一致だけ**（期間内の素の記録＝単発のブロンコ等を引き込まない。フォールバックは非項目制の会だけ）。`msessStatus`は項目制でも done/skip/missed の定義を変えず `submittedPids/inProgressPids/items` を**追加**で返す（皆勤賞・件数・督促は従来のまま）。staff専用: 特設ページ`V.mp`（`goMSessPage`・kgセル即保存`mpCell`・分/秒は`mpCellTDirty/mpCellTOut`でフォーカスがペアの外へ出た時に`mpCellT`で1回保存・免除`mpExempt`（理由の下書きはstate）・提出`mpSubmit`・列モード・保存後は行だけ局所更新`mpRefreshRow`＋表示中なら`mpRerender`）、`msessConsolidatePlan/Apply`＋`goMSessConvert/doMSessConvert`（既存会の特設ページ化=2段階・冪等・Undo）、`reqQueue().msessPend`。player: `physEntry`/`physSessFor(mode)`（測る項目に合う項目制の会が開いていれば`showPhysSessForm`）・`mySessCardHtml`（開いている会ごと）・`mySessTodo`（配列）・`doPhysSess`（差分送信・画面に出した元値`data-orig`と同じなら送らない・提出済み/未統合はupdateFn内でサーバー最新から再判定）。**項目制の記録は player の修正/削除・staff の記録追加/一括/編集/削除から直接触らない**（振り替えるか弾く）。
...
- 新規マークアップは**生hex/rgba禁止・var(--token)のみ**。`sync_check.py --residue`が違反>0でexit 1（機械ゲート）。共通関数はsync_manifest登録（identical 170＋variant 15）＝**網羅リストはsync_manifest.jsonが正典**。触ったら`sync_check.py`が緑になるまで全ファイル揃える（variantの意図的変更は`--update`）。`sync_check.py`はidentical名の同一ファイル内二重定義もNG検出する（P1a・機械ゲート）。
...
```bash
# ① 全テスト一括（構文チェック込み・69本を対象サイト別に84実行）
