...
- **フェーズ1（player/index.html トレーニング入力ツールの作り替え）完了・全push済み**（commit 4517acf / 74c44d0）。
  ...
  - 結果: ステップ1=15/15+8/8、ステップ3=17/17、ステップ2=10/10、ステップ4=13/13+8/8 パス。括弧バランス検査済み。
...
- テスト用選手「テスト選手」(CTB/1年, id=1782426506586相当, note=動作確認用) を本番に1名追加済み（削除可）。

## 次にやること（ユーザー合意済みの順番：3→1→4）
1. **【最優先】素の `sv()` 掃討（データ安全の総点検）** — 今回の名簿全消し事故と同じ火種。まず `sv('chart')`（`staff:1594` / `trainer:398` の saveChart、injId無し時フォールバック）を `svSafeUpdate(upsert)` へ。さらに4ファイル全体を `grep` で洗い、素の `sv(` を一掃。task_c3a91936（チップ）あり。詳細メモリ: project_plain_sv_clobber。
2. **実機スモーク** — テスト選手でフェーズ1を実機確認（プリセット入力・0kg/0回保存・連打ガード・通信断時の温存＆ボタン復帰・端末跨ぎ下書き復元）。模擬実行は通過済み。
3. **次フェーズ＝他入力画面の刷新** — 同じ安全土台で コンディション入力 → **怪我報告（痛み0が入力できないバグ=`if(!pain)`修正）** → フィジカル/体組成 を順次。共通ヘルパー(`isFilled`/`svSafeSeq`/`guardSubmit`)を staff/trainer/coach へ横展開し4サイトの一貫性回復。
