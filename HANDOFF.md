...
- **🆕Phase 9 進行中（プラン: `/Users/nakayamarinnin/.claude/plans/users-nakayamarinnin-documents-0701-070-luminous-hopcroft.md`＝必読・これが正）。9-A/9-B/9-C/9-D完了、残るはプランの「### 仕上げ」＋任意のマイデータ⑥試合スタッツ**。承認は「進めて！」で取得済み＝要件・設計確定・再質問不要。同プランに沿って着手してよい。
  ...
  - **⏭次にやる候補（どちらもプラン内・優先順はユーザーに確認）**:
    - **(a) 仕上げ＝実ファイル5点の初期投入（プラン88-90行目）**: `0701`/`0703`練習GPS(xlsx)・`0704`試合GPS A/B(Excel印刷PDF)・スタッツA/B(PDF)。GPS xlsxはstaffのExcel選択で直接投入可。**GPS PDF2枚とスタッツPDF2枚はグリフ埋込で機械抽出不能→ClaudeがPDFを画像化(`qlmanage -t`は1ページ目のみ・複数ページは要pypdf分割orプラン内数値をTSV化)して読み、staff貼付欄に入れるTSVへ変換**。列はGPS=`GPS_COLS`/スタッツ=`MS_COLS`順。名前は姓のみ・同姓は照合画面で選択。**⚠️プレビュー環境=本番Firestore接続なので、投入は本番staff画面(https://rinto1129.github.io/rugby-manager/staff/)からユーザーが行うのが安全。Claudeがブラウザで投入する場合は本番に書くと自覚し慎重に**。
