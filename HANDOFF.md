...
## 最終更新
- 日時: 2026-07-09（**🆕仕上げ「方法1」確定＝ユーザーが本番staff画面から自分で投入する。取込パッケージ（ガイド＋4TSV＋練習xlsx2シート）を実staffパーサで全6セッションend-to-end再検証してOK・ガイドの成功率説明式の誤りを1行修正。投入の実行はユーザー・Claudeは照合質問に待機。作業はコミット済み・破棄なし。**）
- 更新者: Claude

## 🔴 次セッションが最初にやること

- **✅質問1（実データ投入は誰が）＝回答済み「方法1＝(a)ユーザー自身が本番staff画面から投入」**。Claudeは本番Firestoreに直接書かない。ユーザーが照合（同姓/異体字）で詰まったら都度サポートする体制。
- **⏳質問2（push可否）＝未回答のまま**。未pushコミットは現在9件（9-A `3cb8825`／9-B `9f174fd`／9-C `82869de`／9-D `4f80268`＋HANDOFF更新群）。既定方針「pushは明示指示があるまでしない」に従い保留。ユーザーがpushと言ったら`git push`。
- **📦仕上げ取込パッケージ（検証完了・場所=`/Users/nakayamarinnin/Documents/福大/スタッツ/取込用TSV/`）**: `00_取込ガイド.txt`＋①GPS試合工大A(17名)②GPS試合工大B(23名)③スタッツvsFIT_A(18名)④スタッツvsFIT_B(23名)。練習GPSは`0701~0703.xlsx`のシート`0701`(15名)/`0703`(15名)をstaffで直接選択。**このセッションで実staffパーサ（parseGpsGrid/parseMstatGrid/parseGpsTSV/parseMstatTSV＋GPS_STOPNAMES＋日付自動）を使い全6セッションを再現検証済み**＝ヘッダ行(`名前`/`合計`)はSTOPNAMESで除外・`-`/`%`はgpsNumが無視・km→m・練習は15名ちょうどで打切り・シート名`0701`→日付`2026-07-01`自動。→ユーザーが投入すれば通る状態。**投入自体はまだ未実施**（本番Firestoreは無変更）。
  - **人が判断する必要がある点（照合画面）**: 同姓＝古賀×2/生力/和合、異体字＝﨑迫(U+FA11)⇔崎迫(U+5D0E)/渡邉(U+9089)⇔渡邊(U+908A)。auto照合できない行はambig(select)かnone→ユーザーが名簿から選ぶ。gmap学習は姓一意/未登録のみ（曖昧姓は毎回選択）。
