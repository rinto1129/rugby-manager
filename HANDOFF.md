...
- **🆕🆕Phase 9 進行中（プラン: `/Users/nakayamarinnin/.claude/plans/users-nakayamarinnin-documents-0701-070-luminous-hopcroft.md`＝必読・これが正）。9-A完了、次は9-B**。承認は「進めて！」で取得済み＝要件・設計確定・再質問不要。9-B以降も同プランに沿って着手してよい。
  ...
  - **プレビュー環境（重要・9-A実証）**: プレビューハーネスは起動プロセスを**サンドボックス化しscratchpad配下しか読めない**（リポジトリ直下を直接配信すると404）。→`.claude/launch.json`のdirectoryを現セッションscratchpadの`previewroot`に向け、`cp staff/index.html <previewroot>/staff/index.html`で同期してから起動/リロード。**このセッションのpreviewroot**: `/private/tmp/claude-501/-Users-nakayamarinnin-Documents-rugby-manager/ed76c14b-0d83-4e6f-996c-fb461fd4a276/scratchpad/previewroot`（次セッションは自セッションのscratchpadに作り直す）。Firestoreは**この環境では接続可**（9-Aで実保存成功→テストデータは削除済み）。実機テストで本番Firestoreに書いたら必ず消すこと。
