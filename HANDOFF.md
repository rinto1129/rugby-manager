...
- **✅✅✅✅✅ coach「3体レビュー未適用分」の高優先度5件（C1〜C4+U3）実装・push完了（2026-07-06・このセッション、コミット`29d7f07`）。** ユーザーが「coach未適用レビューをお願い」と依頼→未適用リストを提示→「高優先度を順番に全部」で着手を承認。plan `4-player-staff-trainer-coach-1-ux-merry-harbor.md`末尾のC1-C4/U3を実装。**実機確認はまだ＝次回ユーザーに確認依頼すること。**
...
- **⚠️ player/index.htmlに未commit・未pushの変更あり（2026-07-06発見）**: `SK`/`D`に`std:'rm_standards'`キーが追加されているが、対応する機能・他ファイルへの反映が一切ない孤立した追加。このセッションの作業ではない。`ps aux`で確認したところ本プロジェクトに対し複数のClaudeセッションが同時稼働しており、別ウィンドウの作業中内容の可能性が高いと判断し、ユーザー確認の上でpush対象から除外した（HANDOFF.md・coach/index.htmlのみcommit&push＝`29d7f07`）。次回セッション開始時、この変更がまだ残っていれば、そのセッションで完成させるか破棄するかユーザーに確認すること。`ld()`は`Object.keys(SK)`でFirestoreへ読みに行くため、`std`キーがあると存在しない`appdata/std`への無害な読み取りが毎回発生する（実害なし）。
