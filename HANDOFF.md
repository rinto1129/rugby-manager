...
- **🆕🆕 次にやること＝怪我×リハビリ連携 Phase2「不足ゲージ」に着手決定（2026-07-05・ユーザー選択）。プラン本体＝`/Users/nakayamarinnin/.claude/plans/wise-drifting-moonbeam.md`（plan modeでユーザー承認済み・着手前に必読）。まだ1行も実装していない（クリア直前）。** 要点: staff＋trainer両方のカルテ評価タブに、`chart.metrics`(Phase1で作成)の達成率%・「あと◯◯」を**Chart.js不使用のCSS/SVGネオンリング**で見える化する（読み取り＋描画のみ・書込ゼロ＝回帰リスク最小）。ユーザー確定事項2つ: ①「受傷前」基準は最古evalで代用（ラベル「初回評価比(参考)」明示）②範囲はstaff＋trainer両方（trainerは表示のみ）。実装の足場: 読み手ヘルパー(`metricEvalRef`/`readEvalMetric`/`latestEvalForMetric`/`symPct`/`metricSeries`/`sparklineSVG`)は**trainer:2687〜に実装済み**→staffへverbatim移植して流用。新設は`resolveMetricBase`/`gaugeCalc`/`gaugeSVG`/`renderDeficitGauges`+MMTドット(両ファイルバイト一致)。配線=`renderChartEval`(staff:1987/trainer:1631)。脳震盪chart・metrics空は`''`返し。既知の限界・検証手順(JXA構文+モック全分岐)はプラン参照。
