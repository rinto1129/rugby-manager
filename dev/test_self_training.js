...
print('--- selfTimedKm フォールバック正規化（表記ゆらぎ吸収・完全一致は温存） ---');
ok('完全一致 ブロンコ=1.2',selfTimedKm('ブロンコ')===1.2);
ok('完全一致 1K（1,000m走）=1.0',selfTimedKm('1K（1,000m走）')===1.0);
ok('ブロンコ含む文字列=1.2',selfTimedKm('ブロンコテスト')===1.2);
ok('英字Bronco=1.2',selfTimedKm('Bronco')===1.2);
ok('全角ＢＲＯＮＣＯ=1.2(NFKC)',selfTimedKm('ＢＲＯＮＣＯ')===1.2);
ok('1k=1.0',selfTimedKm('1k')===1.0);
ok('1km=1.0',selfTimedKm('1km')===1.0);
ok('1000m走=1.0',selfTimedKm('1000m走')===1.0);
ok('全角１ｋｍ=1.0(NFKC)',selfTimedKm('１ｋｍ')===1.0);
ok('通常種目ランニングは null',selfTimedKm('ランニング')===null);
ok('スクワットは null(誤検出しない)',selfTimedKm('スクワット')===null);
ok('10km走は null(1kと誤検出しない)',selfTimedKm('10km走')===null);
ok('空文字は null',selfTimedKm('')===null);
__alerts.length=0;
